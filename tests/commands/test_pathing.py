...
class TestPathLiteral:
    ...
    def test_literal_component(self, tmp_path):
        ...
        result = util.create_path_from_character(char, base_path=str(tmp_path), hierarchy='Dudes')
        assert result == str(tmp_path.joinpath('Dudes'))
...
class TestTagPresence:
    ...
    def test_tag_presence_false(self, tmp_path):
        ...
        result = util.create_path_from_character(char, base_path=str(tmp_path), hierarchy='{school?Bros}')
    ...
    def test_tag_presence_true(self, tmp_path):
        ...
        tmp_path.joinpath('Bros').mkdir()
        result = util.create_path_from_character(char, base_path=str(tmp_path), hierarchy='{school?Bros}')
        assert result == str(tmp_path.joinpath('Bros'))

    def test_group_presence_true(self, tmp_path):
        char = npc.Character(type=['human'], groups=['some frat'])
        tmp_path.joinpath('Bros').mkdir()
        result = util.create_path_from_character(char, base_path=str(tmp_path), hierarchy='{groups?Bros}')
        assert result == str(tmp_path.joinpath('Bros'))

    def test_foreign_presence_true(self, tmp_path):
        char = npc.Character(type=['human'], foreign=['over there'])
        tmp_path.joinpath('Bros').mkdir()
        result = util.create_path_from_character(char, base_path=str(tmp_path), hierarchy='{foreign?Bros}')
        assert result == str(tmp_path.joinpath('Bros'))
    ...
    def test_type_presence_true(self, tmp_path):
        char = npc.Character(type=['human'])
        tmp_path.joinpath('Bros').mkdir()
        result = util.create_path_from_character(char, base_path=str(tmp_path), hierarchy='{type?Bros}')
        assert result == str(tmp_path.joinpath('Bros'))

    def test_ranks_presence_true(self, tmp_path):
        char = npc.Character(type=['human'], ranks={'some frat': ['handstand guy']})
        tmp_path.joinpath('Bros').mkdir()
        result = util.create_path_from_character(char, base_path=str(tmp_path), hierarchy='{ranks?Bros}')
        assert result == str(tmp_path.joinpath('Bros'))

    def test_translated_presence_true(self, tmp_path):
        char = npc.Character(type=['changeling'], motley=['Kickasso'])
        tmp_path.joinpath('Bros').mkdir()
        result = util.create_path_from_character(char, base_path=str(tmp_path), hierarchy='{type-unit?Bros}')
        assert result == str(tmp_path.joinpath('Bros'))
...
class TestTagInsertion:
    ...
    def test_tag_no_value(self, tmp_path):
        ...
        result = util.create_path_from_character(char, base_path=str(tmp_path), hierarchy='{school}')
        assert result == str(tmp_path)
    ...
    def test_tag_no_value(self, tmp_path):
        ...
        tmp_path.joinpath('U of Bros').mkdir()
        result = util.create_path_from_character(char, base_path=str(tmp_path), hierarchy='{school}')
        assert result == str(tmp_path.joinpath('U of Bros'))

    def test_type_tag_value_substitution(self, tmp_path):
        char = npc.Character(type=['human'])
        tmp_path.joinpath('Humans').mkdir()
        result = util.create_path_from_character(char, base_path=str(tmp_path), hierarchy='{type}')
        assert result == str(tmp_path.joinpath('Humans'))

    def test_translated_tags(self, tmp_path):
        char = npc.Character(type=['changeling'], motley=['Kickasso'])
        tmp_path.joinpath('Kickasso').mkdir()
        result = util.create_path_from_character(char, base_path=str(tmp_path), hierarchy='{type-unit}')
        assert result == str(tmp_path.joinpath('Kickasso'))
    ...
    def test_single_group(self, tmp_path):
        ...
        second = first / 'Second'
        second.mkdir()
        result = util.create_path_from_character(char, base_path=str(tmp_path), hierarchy='{group}')
        assert result == str(tmp_path.joinpath('First'))
    ...
    def test_group_rank(self, tmp_path):
        ...
        result = util.create_path_from_character(char, base_path=str(tmp_path), hierarchy='{group}/{ranks}')
        assert result == str(tmp_path.joinpath('First').joinpath('A'))
    ...
    def test_group_folders(self, tmp_path):
        ...
        second = first / 'Second'
        second.mkdir()
        result = util.create_path_from_character(char, base_path=str(tmp_path), hierarchy='{groups}')
        assert result == str(tmp_path.joinpath('First').joinpath('Second'))
    ...
    def test_groups_and_ranks(self, tmp_path):
        ...
        third = first / 'A'
        third.mkdir()
        result = util.create_path_from_character(char, base_path=str(tmp_path), hierarchy='{groups+ranks}')
        assert result == str(tmp_path.joinpath('First').joinpath('A'))
    ...
    def test_location_exists_with_foreign(self, tmp_path):
        ...
        result = util.create_path_from_character(char, base_path=str(tmp_path), hierarchy='{locations}')
        assert result == str(tmp_path.joinpath('over here'))
    ...
    def test_location_not_exist_and_foreign(self, tmp_path):
        ...
        result = util.create_path_from_character(char, base_path=str(tmp_path), hierarchy='{locations}')
        assert result == str(tmp_path.joinpath('way the heck over there'))

    def test_configured_missing_value(self, tmp_path):
        char = npc.Character(type=['changeling'])
        tmp_path.joinpath('Courtless').mkdir()
        result = util.create_path_from_character(char, base_path=str(tmp_path), hierarchy='{type-social}')
        assert result == str(tmp_path.joinpath('Courtless'))
