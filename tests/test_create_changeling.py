...
def test_adds_group_tags(campaign):
    npc.commands.create_changeling('changeling mann', 'Beast', 'Hunterheart', groups=['fork', 'spoon'])
    data = campaign.get_character_data('changeling mann.nwod')
    assert data['group'] == ['fork', 'spoon']

def test_duplicate_character(campaign):
    npc.commands.create_changeling('changeling mann', 'Beast', 'Hunterheart')
    result = npc.commands.create_changeling('changeling mann', 'Beast', 'Hunterheart')
...
def test_adds_court(campaign):
    npc.commands.create_changeling('changeling mann', 'Beast', 'Hunterheart', court='Summer')
    data = campaign.get_character_data('changeling mann.nwod')
