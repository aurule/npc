...
def test_move_by_type(campaign, argparser, new_path):
    campaign.populate_from_fixture_dir(['reorg', 'by_type'])
    args = argparser.parse_args(['reorg'])
    result = npc.commands.reorg(args)
...
def test_move_by_group(campaign, argparser, new_path):
    ...
    args = argparser.parse_args(['reorg'])
    npc.commands.reorg(args)
    character = campaign.get_character(new_path)
    assert character.check()

def test_partial_tree(campaign, argparser):
...
class TestPurge:
    def test_removes_directories(self, campaign, argparser):
    ...
    def test_preserves_directories(self, campaign, argparser):
...
class TestChangeling:
    ...
    def test_move_by_court(self, campaign, argparser, new_path):
    ...
    def test_move_courtless(self, campaign, argparser):
    ...
    def test_move_by_group(self, campaign, argparser, new_path):
